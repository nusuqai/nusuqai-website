...
const PROMPT_SUGGESTIONS: PromptSuggestion[] = [
  { icon: Package, title: "Browse Products", prompt: "Show me all available products" },
  { icon: Search, title: "Search Items", prompt: "Search for electronics" },
  { icon: ShoppingCart, title: "Check Inventory", prompt: "What products are in stock?" },
  { icon: Package, title: "Product Details", prompt: "Tell me about product ID 12345" },
];
...
export function ChatModal({ isOpen, onClose }: ChatModalProps) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 truncate max-w-[120px] opacity-70">Click to draft</p>
          </div>
        </div>
        <CopyPlus size={14} className="text-gray-300 group-hover:text-[#0F1E3D] transition-colors" />
      </div>
    </button>
  );
  ...
  const MobileSuggestionChip = ({ item }: { item: PromptSuggestion }) => (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-gray-700 whitespace-nowrap">{item.title}</span>
    </button>
  );
              ...
              <div className="flex-1 flex flex-col bg-gray-50/50 relative min-w-0">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 scroll-smooth">
                  {messages.map((message) => (
                    ...
                    >
                      <div className={`max-w-[85%] sm:max-w-[70%] space-y-1 ${message.type === "user" ? "items-end" : "items-start"} flex flex-col`}>
                        <div
                ...
                <div className="bg-white border-t border-gray-200 p-3 sm:p-5 flex-shrink-0">
                  ...
                  <div className="relative flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 bg-gray-100 text-gray-900 placeholder-gray-500 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0F1E3D]/10 focus:bg-white transition-all border-transparent border focus:border-[#0F1E3D]/20"
                    />
                    <button
                      onClick={handleSend}
                      disabled={!inputValue.trim() || isTyping}
                      className="p-3.5 bg-[#0F1E3D] text-white rounded-xl hover:bg-[#1a2d4d] active:scale-95 disabled:opacity-50 disabled:scale-100 transition-all shadow-md"
                    >
                      <Send size={18} />
                    </button>
                  </div>
